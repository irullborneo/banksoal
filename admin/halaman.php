...
<title>Administrasi | Bank Soal Sistem Kolaborasi Kurkulum 2013</title>
	...
	<nav class="navbar navbar-inverse navbar-fixed-top" role="navigation">
      <div class="container-fluid">
        <div class="navbar-header">
          ...
          <a class="navbar-brand" href="#">Administrasi Bank Soal</a>
        </div>
        <div id="navbar" class="navbar-collapse collapse">
        	<ul class="nav navbar-nav navbar-right">
        	<?php
				$querymenu=mysql_query("SELECT menu, link FROM menu WHERE (login='semua' OR login='admin') AND tempat='atas' ORDER BY level ASC");
				$pagenow=$_GET['p'];
    ...
    <div class="container-fluid">
        ...
        <div class="col-sm-3 col-md-2 sidebar">
        	<?php
				...
				$querymenu2=mysql_query("SELECT menu, link FROM menu WHERE (login='semua' OR login='admin') AND tempat='bawah' ORDER BY level ASC");
				while($menu2=mysql_fetch_array($querymenu2)){
					...
					for($i=0;$i<count($menu);$i++){
						if($link[$i]=="?p=".$pagenow || $link[$i]==$pagenow){
							...
							if($menu[$i]=="Guru"){
								$total=totalbadge("guru");
								if($total>0){
									echo "<span class=\"badge\">$total</span>";
								}
							}
							else if($menu[$i]=="Siswa"){
								$total=totalbadge("Siswa");
								if($total>0){
									echo "<span class=\"badge\">$total</span>";
								}
							}
							else if($menu[$i]=="Paket Pelajaran"){
