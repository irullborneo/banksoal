	...
	function ceklogin(form){
		...
		if(username=="" && password==""){
			...
			$.ajax({
				...
				url:"include/kirim.php?kirim=ceklogin",
...
<div class="form-login">
	<form class="form-signin" method="post" onsubmit="return ceklogin(this)" role="form">
        <h2 class="form-signin-heading">Tutor Masuk</h2>
        <label for="nis" class="sr-only">Username</label>
        <input type="text" name="username" id="username" class="form-control" placeholder="Username" autofocus />
        <label for="password" class="sr-only">Password</label>
        <input type="password" name="password" id="password" class="form-control" placeholder="Password" />
        <input class="btn btn-lg btn-primary btn-block" type="submit" value="Masuk" />
        <div style="margin-top:10px;"><a href="./aktivasi-akun.php" class="text-info" style="font-size:16px">Aktivasi akun tutor</a></div>
	</form>
