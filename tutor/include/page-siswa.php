...
<?php
	...
	echo "<div class=\"row\">
		";
		$querypaket=mysql_query("SELECT p.paket, s.id_paket FROM siswa as s, paket as p WHERE s.id_paket=p.id_paket AND s.id_sekolah='$admin[id_sekolah]' GROUP BY s.id_paket");
		while($paket=mysql_fetch_array($querypaket)){
			echo "<div class=\"col-sm-6\">
				...
				<table class=\"table table-striped table-hover\">
					...
					<tbody  id=\"tabel-$paket[1]-$admin[id_sekolah]\">
			";
			$querysiswa=mysql_query("SELECT id_siswa, nama, jenis_kelamin, tanggal_lahir FROM siswa WHERE id_paket=$paket[1] AND id_sekolah='$admin[id_sekolah]' ORDER BY id_siswa DESC limit 5");
			...
			";
			$banyaksiswa=mysql_num_rows(mysql_query("SELECT id_siswa, nama, jenis_kelamin, tanggal_lahir FROM siswa WHERE id_paket=$paket[1] AND id_sekolah='$admin[id_sekolah]' ORDER BY id_siswa DESC"));
			echo "Ada $banyaksiswa data siswa<br /><br />";
			if($banyaksiswa>5){
				echo "<button type=\"button\" class=\"btn btn-primary lihat-siswa\" id='siswa-$paket[1]-$admin[id_sekolah]'>Lihat Semua</button>
				<button type=\"button\" class=\"btn btn-primary kembali-siswa\" id=\"kembali-$paket[1]-$admin[id_sekolah]\" style=\"display:none\">Kembali</button>
				";
			}
			echo "
			</div>
