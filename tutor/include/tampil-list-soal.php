...
<?php
	...
	while($soal=mysql_fetch_array($querysoal)){
		...
		";
		$idguru=mysql_fetch_array(mysql_query("SELECT pp.id_guru, sp.penulis FROM soal_pelajaran as sp, paket_pelajaran as pp WHERE sp.id_paket_pelajaran=pp.id_paket_pelajaran AND sp.id_soal_pelajaran='$_GET[idpelajaran]'"));
		if($idguru[0]==$admin['id_guru'] || $idguru[1]==$admin['id_guru']){
		echo "
