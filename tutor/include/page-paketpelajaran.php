       	...
       	<?php
			...
			while($paketpelajaran=mysql_fetch_array($querypaketpelajaran)){
				...
				while($paket1=mysql_fetch_array($querypaket1)){
					$id=mysql_fetch_array(mysql_query("SELECT pp.id_paket_pelajaran FROM paket_pelajaran as pp, guru as g WHERE pp.id_guru=g.id_guru AND g.id_sekolah='$admin[id_sekolah]' AND pp.id_paket=$paket1[id_paket] AND pp.id_pelajaran=$paketpelajaran[0]"));
					$total=mysql_num_rows(mysql_query("SELECT * FROM paket_pelajaran as pp, guru as g WHERE pp.id_guru=g.id_guru AND g.id_sekolah='$admin[id_sekolah]' AND pp.id_paket=$paket1[id_paket] AND pp.id_pelajaran=$paketpelajaran[0]"));
					if($total>=1){
						echo "<td class=\"label-primary paketpelajaran\" style=\"color:#fff; cursor:pointer\" id=\"paket-$id[0]\" data-toggle=\"modal\" data-target=\"#form-lihat-data\">&nbsp;</td>";
					}
					else{
