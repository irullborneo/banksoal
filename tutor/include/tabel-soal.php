	...
	$querysoal=mysql_query("SELECT sp.id_soal_pelajaran, sp.tgl_input, p.paket, mp.mata_pelajaran, g.nama, sp.penulis, g.id_guru, sp.untuk FROM soal_pelajaran as sp, paket_pelajaran as pp, paket as p, mata_pelajaran as mp, guru as g WHERE sp.id_paket_pelajaran=pp.id_paket_pelajaran AND pp.id_paket=p.id_paket AND pp.id_pelajaran=mp.id_pelajaran AND pp.id_guru=g.id_guru ORDER BY $kolom $urut limit $awal,$baris");
			while($soal=mysql_fetch_array($querysoal)){
				...
				echo "
	    	        ...
	    	        <td>
				";
				if($soal[5]==$admin['id_guru'] || $soal[6]==$admin['id_guru']){
					echo "
